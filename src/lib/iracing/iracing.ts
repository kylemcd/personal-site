...
class IRacingRecentRacesError extends Data.TaggedError(
	"IRacingRecentRacesError",
)<Error> {
	message = "Failed to fetch recent races";
}

class IRacingSummaryError extends Data.TaggedError(
	"IRacingSummaryError",
)<Error> {
		...
		fetchFresh({
			...
			Effect.flatMap(({ data: { link } }) =>
				fetchFresh({
					...
					Effect.mapError((error) => new IRacingCarsError(error as Error)),
				),
			),
			Effect.map(({ data }) => {
				return data;
		...
		fetchFresh({
			...
			Effect.flatMap(({ data: { link } }) =>
				fetchFresh({
					...
					Effect.mapError(
						(error) => new IRacingRecentRacesError(error as Error),
					),
				),
			),
